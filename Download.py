    ...
    def search_Pytube(self,link:str)->(list,list,dict):
        ...
        for i in range(3):
            try:
                ...
                self.data[link]= [title, "Wartet"]
                break
            except:
                continue
        else:
            self.Fehler.append(link)
            self.data[link] = ["Fehler", "Video nicht verfügbar/nicht öffentlich"]
    ...
    def playlist(self,link:str)->(list,list,dict):  # Prüft ob Link der Playlist gültig ist
        ...
        try:
            ...
            for video in videos:
                ...
                self.data[video_link] = [title, "Wartet"]
            if self.data == {}:
                self.Links.append(link)
                self.Fehler.append(link)
                self.data[link] = ["Fehler", "Playlist nicht verfügbar/nicht öffentlich"]
        except:
            self.Links.append(link)
            self.Fehler.append(link)
            self.data[link] = ["Fehler",  "Keine Internetverbindung"]
        return self.Links,self.Fehler,self.data
    ...
    def new_vid(self,link:str,max_results:int = 3)->(list,list,dict):
        ...
        for j,ch_name in enumerate(ch_names):
            ...
            try:
                ...
                self.data[f"##{j}##"] = [ch_name, "#####"]
                self.Links.insert(index, f"##{j}##")
            except Exception as e:
                error_count+=1
                if "[Errno 11001]" in str(e):
                    self.data[ch_name] = ["Fehler", "Keine Internetverbindung"]
                else:
                    self.data[ch_name] = ["Fehler", "Unbekannter Kanalname"]
